...
public class MasterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //데이터베이스에서 location_table 테이블의 category 컬럼의 값을 중복 없이 가져와서 List에 삽입
        LocationTable locationTable = new LocationTable(this);
        SQLiteDatabase db = locationTable.getReadableDatabase();
        //select 구문 실행
        Cursor cursor = db.rawQuery("select distinct category from location_table", null);
        //데이터를 저장할 ArrayList 만들기
        ArrayList<String> list = new ArrayList<>();
        //select 구문 순회
        while(cursor.moveToNext()){
            //select에서의 컬럼 (0번이 category)
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                Log.e("category에 저장된 값",category);
